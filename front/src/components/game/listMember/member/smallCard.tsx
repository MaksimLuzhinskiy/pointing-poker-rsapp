...
const SmallCard: FC<ICard> = ({
  ...
  return (
    <div className="smallCard">
      <Wrap>
        <div className="smallCard__avatar">
          <img src={image} alt="" />
        </div>
        <WrapInfo>
          {isYou && <div className="smallCard__ityou">IT'S YOU</div>}
          <div className="smallCard__name">
            {name} {surname}
          </div>
          <div className="smallCard__jobPosition">{jobPosition}</div>
        </WrapInfo>
        {!isYou && (
          <svg
            className="smallCard__kick"
            width="36"
            onClick={clickKick}
            height="36"
            viewBox="0 0 48 48"
